...
ARCH="$(uname -m)"
NAME="PressReady-${VERSION}-macos-${ARCH}"

echo "=== PressReady ${VERSION} — macOS build (${ARCH}) ==="

# --- icon: PNGs -> .icns
ICONSET="build/pressready.iconset"
...
iconutil -c icns "${ICONSET}" -o assets/icons/pressready.icns
echo "[ok] built assets/icons/pressready.icns"

rm -rf build/PressReady dist
$PY -m PyInstaller PressReady.spec --noconfirm --distpath dist --workpath build

# --- dmg
STAGE="build/dmg"
rm -rf "${STAGE}"; mkdir -p "${STAGE}"
cp -R "dist/PressReady.app" "${STAGE}/"
ln -s /Applications "${STAGE}/Applications"
cp README.md LICENSE "${STAGE}/" 2>/dev/null || true

hdiutil create -volname "PressReady ${VERSION}" -srcfolder "${STAGE}" \
